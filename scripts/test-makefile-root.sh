...
assert_plan() {
  ...
  target=$2
  output=$3
  printf '%s' "$output" | grep -Fq "$CHECKOUT" || {
    printf '%s\n' "$scenario $target did not use $CHECKOUT" >&2
    exit 1
  }
...
printf '%s\n' "Makefile root tests passed: 21 target/override cases and 2 MAKEFILE_LIST rejection cases"
